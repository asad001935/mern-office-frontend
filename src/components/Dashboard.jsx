...
export default function Dashboard() {
  ...
  const activeOfficeBoys = data.officeBoys.filter(
    (officeBoy) => !officeBoy.isDeleted,
  ).length;
  ...
  return (
    <>
      {loading && <Loader text="Preparing Dashboard..." />}
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Manage employees, restaurants, and order flow from one place.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          />
          <StatCard
            title="Active Office Boys"
            value={activeOfficeBoys}
            icon={Truck}
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <StatCard
            ...
            value={analytics.mostOrderedItems?.[0]?.itemName || "No data"}
            icon={ClipboardList}
            tone="bg-violet-50 text-violet-700"
          />
          <StatCard
            title="Peak Time"
            value={analytics.peakTime || "No data"}
            icon={Clock3}
                  ...
                  {latestOrders.map((order) => (
                    <tr key={order._id}>
                      <td className="px-5 py-4 font-medium text-slate-800 dark:text-slate-200">
                        {order.userId?.userName || "Employee"}
                      </td>
                ...
                <div key={label}>
                  ...
                  <div className="h-2 rounded-full bg-slate-100 dark:bg-slate-700">
                    <div
                      ...
                      style={{
                        width: `${orders.length ? Math.max((value / orders.length) * 100, 8) : 0}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
            ...
            <div className="mt-4 space-y-3">
              ...
              {(!analytics.mostOrderedItems ||
                ...
                <p className="rounded-lg bg-slate-50 p-4 text-sm text-slate-500 dark:bg-slate-700 dark:text-slate-400">
                  No item analytics yet.
                </p>
              )}
            </div>
          </section>
